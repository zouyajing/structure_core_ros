...
//
	...
	void handleAccel(const ST::AccelerometerEvent &accelEvent)
	{
		...
		imu_.linear_acceleration.z = accelEvent.acceleration().z;
                if(imu_.header.stamp.toSec() <accelEvent.timestamp())
	...
	void handleGyro(const ST::GyroscopeEvent &gyroEvent)
	{
		...
		imu_.angular_velocity.z = gyroEvent.rotationRate().z;
                if(imu_.header.stamp.toSec() <gyroEvent.timestamp())
...
ros::NodeHandle in(n, "imu");
imu_pub_ = in.advertise<sensor_msgs::Imu>("imu", 10);
...
int main(int argc, char **argv) {
    ...
    */
    ...
    //settings.structureCore.visibleAutoExposureEnabled = true;
    ...
    //settings.structureCore.initialVisibleExposure = 0.001f;
    /** @brief The initial visible gain to start streaming with. Can be any number between 1 and 8. */
    //settings.structureCore.initialVisibleGain = 8.0f;
